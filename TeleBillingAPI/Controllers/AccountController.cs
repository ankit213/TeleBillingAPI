...
namespace TeleBillingAPI.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
		...
		#region "Constructor"
		public AccountController(IAccountRepository iAccountRepository, IStringConstant iStringConstant, IEmailSender iEmailSender,IConfiguration configuration) {
		...
		public async Task<IActionResult> Login([FromBody]LoginAC loginAC) {
			...
			try
			{
				...
				if (user != null)
				{
					...
					if (await _iAccountRepository.CheckUserCredentail(user.EmailId, loginAC.Password))
					{
